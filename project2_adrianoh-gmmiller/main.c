...
void initResolution(char *fileName, int height, int width) {
  FILE *fp = fopen(fileName, "r");
  if (fp == NULL) {
    printf("The file name you entered (%s) is invalid.\n", fileName);
    return;
...
int main(int argc, char *argv[]) {
  ...
  int resolutionTool = -1;
  char *input_file = NULL;
  int res = 512;

  // Parse the CLI input!
  while ((opt = getopt(argc, argv, "hmgtf:n:r:")) != -1) {
  ...
  if (resolutionTool > 0) {
    initResolution(input_file, res, res);
  ...
  if (graphics > 0) {
    ...
    exportFramesSimulate(spheres, numSpheres, e, u, v, numLights, lights,
                         numFrames);
  } else if (resolutionTool > 0) {
    float* new_img = (float*)malloc(3 * res * res * sizeof(float));
    sort(spheres, numSpheres, e);
    render(new_img, res, res, e, u, v, numLights, lights);
    // Feel free to replace renderOrig with a render implementation that you are absolutely sure is correct
    renderOrig(img, res, res, e, u, v, numLights, lights);
    for (int i = 0; i < 3 * res * res; i++) {
      if (img[i] != new_img[i]) {
        printf("FAILED RESOLUTION CHECK\n");
        return 0;
      }
    }
    printf("PASSED RESOLUTION CHECK\n");
    return 0;
  } else if (test_tiers > 0) {
