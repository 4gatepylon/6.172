...
move_t tt_move_of(ttRec_t* rec) { return rec->move; }

// getting the score out of the record
score_t tt_score_of(ttRec_t* rec) { return rec->score; }

size_t tt_get_bytes_per_record() { return sizeof(struct ttRec); }
...
void tt_hashtable_put(uint64_t key, int depth, score_t score, int bound_type,
                      move_t move) {
