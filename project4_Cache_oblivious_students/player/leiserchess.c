...
#ifndef PEDANTIC
// disable or enable consistency checking throughout the compiled binary
// if you want to enable, compile without -DPEDANTIC
int CHECK_REP;
...
#ifndef PEDANTIC
  fprintf(OUT,
          "info dump      - Dump an internal data structure if a dump routine "
          "is defined for it.\n");
  fprintf(OUT, "info             Sample usage:\n");
  fprintf(OUT,
          "info                 dump myrand: dump the state of the myrand data "
          "structure, if DUMP_DS(myrand) is defined in the codebase\n");
#endif /* ! PEDANTIC */
...
int main(int argc, char* argv[]) {
  ...
  gme->pawn_locs[0][0] = 70;
  gme->pawn_locs[0][1] = 88;
  gme->pawn_locs[0][2] = 119;
  gme->pawn_locs[0][3] = 132;

  gme->pawn_locs[1][0] = 122;
  gme->pawn_locs[1][1] = 136;
  gme->pawn_locs[1][2] = 155;
  gme->pawn_locs[1][3] = 183;
  ...
  while (true) {
    ...
    if (fgets(istr, 20478, IN) != NULL) {
        ...
        if (is_ILLEGAL(victims)) {
          ...
          fprintf(OUT,
                  "move victims -1\n");  // DO NOT CHANGE THIS LINE!!! needs to
                                         // be this format for the autotuner
        } else {
          ix++;
          display(&gme[ix]);
          fprintf(OUT, "move victims %d\n",
                  victims.zapped_count);  // DO NOT CHANGE THIS LINE!!! needs to
                                          // be this format for the autotuner
        }
        continue;
      }
      ...
      if (strcmp(tok[0], "status") == 0) {
        if (gme[ix].victims.zapped_count > 0 &&
            gme[ix].victims.zapped[0].typ == MONARCH) {
          if (gme[ix].victims.zapped[0].color == BLACK)
      ...
      if (strcmp(tok[0], "perft") == 0) {  // Sanity check for move generator
        // Correct output to depth 4 from startpos
        // info perft  0 1
        // info perft  1 44
        // info perft  2 1925
        // info perft  3 87172
        // info perft  4 3906989
