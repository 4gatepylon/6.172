...
static void evaluateMove(moveEvaluationResult* result, searchNode* node,
                         move_t mv, move_t killer_a, move_t killer_b,
  ...
  // Check whether the game is a game over position - either someone was shot or
  // it's in our closing book.
  if (is_end_game_position(&(result->next_node.position), node->pov,
                           node->ply)) {
  ...
  if (victims.zapped_count > 0) {
    ...
    for (int i = 0; i < victims.zapped_count; i++) {
      if (victims.zapped[i].color != node->fake_color_to_move) {
        blunder = false;
        break;
      }
    }
...
static int get_sortable_move_list(searchNode* node, sortable_move_t* move_list,
  ...
  for (int mv_index = 0; mv_index < num_of_moves; mv_index++) {
    ...
    if (move_eq(mv, hash_table_move)) {
      ...
      int ot = (node->position.board[fs].pawn_ori + ro) % NUM_ORI;
