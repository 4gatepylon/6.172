...
int main(int argc, char* argv[]) {	
  ...
  while (fgets(s, 4096, f) != NULL) {	
    ...
    for (i = 0; i < tc; i++) {	
      if (Xisdigit(tok[i][0]) && tok[i][1] != '}' && tok[i][2] != '}' &&	
          tok[i][3] != '}' && tok[i][4] != '}') {	
  ...
  printf(	
      "      TIME       RATIO    log(r)     NODES    log(r)  ave DEPTH    "	
      "GAMES   PLAYER\n");	
  printf(	
      " ---------  ----------  --------  --------  --------  ---------  "	
      "-------   %s\n",	
  ...
  for (i = 0; i < pc; i++) {	
    printf("%10.4f  %10.3f  %8.3f  %8.3f  %8.3f  %9.4f  %7d   %s\n",	
           players[i].ts, players[i].ts / players[rix].ts,	
           log(players[i].ts / players[rix].ts), players[i].nm,	
           log(players[i].nm / players[rix].nm),	
           players[i].depth / (double)players[i].moves, players[i].games,	
           players[i].name);	
  }	
